...
# Multiple zones/routes, each with its own station list. Every route is
# independently synthetic (same generator, different station sequence and
# distances), so a train's route_id + station_seq together fully describe
# its position — no change needed to the historical-average or delay_so_far
# feature logic in ml/features.py, since those group by (train_no, ...) and
# train_no is unique across all routes.
ROUTES = {
    "NJP-BJU": [
    ...
    "GHY-APDJ": [
        ("GHY", "Guwahati", 0),
        ("RNY", "Rangiya Jn", 30),
        ("NBQ", "New Bongaigaon", 180),
        ("KOJ", "Kokrajhar", 220),
        ("APDJ", "Alipurduar Jn", 300),
    ],
    "HWH-MGS": [
        ("HWH", "Howrah", 0),
        ("ASN", "Asansol", 200),
...
def simulate_train(train_no: str, base_speed_kmph: float, route_id: str):
    stations = ROUTES[route_id]
    rows = []
...
def main():
    ...
    for route_id in ROUTES:
        for _ in range(NUM_TRAINS_PER_ROUTE):
            train_no = f"1{2000 + t}"
            base_speed = RNG.uniform(45, 75)  # kmph, varies by train type
            all_rows.extend(simulate_train(train_no, base_speed, route_id))
            t += 1
    ...
    total_trains = len(ROUTES) * NUM_TRAINS_PER_ROUTE
    print(f"Generated {len(df)} rows for {total_trains} trains across {len(ROUTES)} routes -> {out_path}")
